...
(* idea: keep a heap of the tower so far, weightiest athlete at the root.
	...
	with this difference that we need to keep the "prev. polygons depths" since we're replacing max. weight by a smaller weight but still
...
int main(int argc, char *argv[])
{
	if (argc == 1)	/* empty set (of athletes) */
		...
		printf("Usage: rewot.exe (<weight> <strength>)*");
	return 0;
}
