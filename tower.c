...
do {
	...
	if (xi.s >= S[j]) {	(* xi can support the tallest tower so far: max. height++ *)
		S[m] = xi.w + S[j]
		m++
	} while (j--) {	(* not the tallest towers: must check wether a lighter tower results *)
...
do {
	...
	U = sorted T[p, m-1] (ascending T[i])	(* must update taller towers before the shorter *)
	if ((k = |U|-1) >= 0) {	(* there are towers-so-far that xi can support *)
		if (T[U[k]] == m-1) {	(* add an additional height *)
			S[m] = xi.w + S[m-1]
			m++
			while (k--) {
				if (xi.w + S[T[U[k]]] < S[T[U[k]]+1])
					S[T[U[k]]+1] = xi.w + S[T[U[k]]]
			}
		} else do {
				if (xi.w + S[T[U[k]]] < S[T[U[k]]+1])
					S[T[U[k]]+1] = xi.w + S[T[U[k]]]
