...
i = n-1	(* suppose n > 1 *)
m = 1	(* m = max. tower height so far + 1 *)
do {
	j = m
	if (xi.s >= S[j-1] && xi.w + S[j-1] < S[j]) {
		m++
...
m-1 = height of the tallest towers over x0, ... xn-1
...
/* Note: if xj.w+xj.s >= xi.w+xi.s and (xi, xj) is a tower, then (xj, xi) is also a tower with no less remaining weight-room = min(xj.s - xi.w, xi.s), and if (xj, xi) is no tower then neither is (xi, xj): there is no point in placing xj after xi, regardless of (xi.w > xj.w => xi.s > xj.s) => xi.w + xi.s > xj.w + xj.s. Conclusion: the athlete are ordered by weight + strength. */
...
void radix_sort(int l, int r, int b)	/* sort by mass + strength, the output
											array is such that i < j => !(x[j].w+x[j].s > x[i].w+x[i].s) */
{
	if (l < r) {
		int l0 = l, r0 = r;
...
int f(int n) {
	...
	while (r >= 0) {
		if (r < l) {	/* 0 <= r < l: no out-of-bounds access, below */
			/* a[r] is the lightest athlete who can bear weight w */
			h++;
			w += x[r].w;
			l = 0;
			r--;
